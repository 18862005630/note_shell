...
查找suid文件：$ ls -l | grep '^...s'
查找suid和guid：$ ls -l | grep '^...s..s'
...
例如，如果希望设置guid，那么必须要让该用户组具有执行权限。

如果想要对文件login[它当前所具有的权限为rwx rw- r-- (741)]设置suid，可在使用chmod命令时在该权限数字的前面加上一个4，即chmod 4741，这将使该文件的权限变为r w s rw- r - -。
$ chmod 4741 login
...
chmod 4754   rws r-x r- -   文件被设置了s u i d，文件属主具有读、写和执行的权限，属组用户具有读和执行的权限，用户具有读权限

4、以符号方式设置UID
chmod u+s <filename>; //设置suid
chmod ug+s <filename>; //设置suid和guid
